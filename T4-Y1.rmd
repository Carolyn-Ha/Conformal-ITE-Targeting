...
(1) df_T4: "T4_내일배움카드_들어본_경험"이 있는 사람들만 뽑기
```{r}
# Y - (임금) 취업 여부 => 이것만 활용
selected_cols_T4 <- grep("^(X1_|X2_|X3_|Y_임금근로자_첫_일자리|T4_)", names(data), value = TRUE)

# 행정 인턴제 들어본 경험이 있는 경우만 filtering
df_T4 <- data %>%
  select(all_of(selected_cols_T4)) %>%
  filter(T4_내일배움카드_들어본_경험 == 1)
```
...
levels(df_T4$X1_연령대)
levels(df_T4$X1_부모소득구간)

# NA 개수 여전히 있는지 확인 (X1_부모학력은 그대로 두기로 함)
sum(is.na(df_T4$X1_부모학력))
```
...
(1) raw data 기준
```{r}
# 1. covariates만 선택: Y_, T4_ 으로 시작하는 변수 제외
covariates <- names(df_T4)
covariates <- covariates[!grepl("^Y_|^T4_", covariates)]
...
ps_model <- glm(fml_weighted, data = df_T4, family = binomial)
ps <- predict(ps_model, type = "response")

# 4. ATE 기반 inverse propensity weights 계산
df_T4$weights <- ifelse(df_T4[[treat_col]] == 1,
                        1 / ps,
                        1 / (1 - ps))
...
# 1. 변수 선택
covariates <- names(df_T4)
covariates <- covariates[!grepl("^Y_|^T4_|^weights", covariates)]
...
# 5. Plot용 변수 생성
df_T4$ps <- ps
df_T4$treat_label <- ifelse(df_T4[[treat_col]] == 1, "Treated", "Control")

# 6. 시각화
# ggplot(df_T4, aes(x = ps, fill = treat_label)) +
...
```{r}
ggplot(df_T4, aes(x = ps)) +
  geom_density(data = subset(df_T4, treat_label == "Treated"),
               aes(color = "Treated", linetype = "Treated"), size = 1.2) +
  geom_density(data = subset(df_T4, treat_label == "Control"),
...
df_T4[[treat_col]] <- factor(df_T4[[treat_col]],
                             levels = c(0, 1),
...
df_T4$ps <- pred_probs[, "Treated"]  # "Treated"에 해당하는 확률 사용

# 라벨 생성
df_T4$treat_label <- ifelse(df_T4[[treat_col]] == "Treated", "Treated", "Control")
...
(2) 최종 분석에 사용할 df_T4_final 생성
```{r}
quantile(df_T4$ps, probs = c(0, 0.01, 0.05, 0.1, 0.9, 0.95, 0.99, 1))
df_cf <- df_T4[df_T4$ps >= 0.01 & df_T4$ps <= 0.99, ]

# 1. 안 쓰는 변수 제거: weights, ps, treat_label, T4_내일배움카드_들어본_경험
drop_vars <- c("weights", "ps", "treat_label", "T4_내일배움카드_들어본_경험")
...
(3) 최종 df(df_cf_T1) 저장
```{r}
file_path = "C:/Users/1004c/Desktop/통계청 논문 공모전/Data/T4_Y1/df_cf_T4.csv"

write_excel_csv(df_cf, file_path)
```

```{r}
table(df_cf$T4_내일배움카드_참여경험)
```


# ITE - CQR
```{r}
df_cf <- read_csv("C:/Users/1004c/Desktop/통계청 논문 공모전/Data/T4_Y1/df_cf_T4.csv")
```
...
T <- df_cf$T4_내일배움카드_참여경험
T <- ifelse(T == "Treated", 1, 0)
Y <- df_cf$Y_임금근로자_첫_일자리
# Y <- as.factor(Y)
X <- df_cf[, setdiff(names(df_cf), c("T4_내일배움카드_참여경험", "Y_임금근로자_첫_일자리"))]
...
- result_df 저장
```{r}
file_path = "C:/Users/1004c/Desktop/통계청 논문 공모전/Data/T4_Y1/T4_mean_RF_reg.csv"
